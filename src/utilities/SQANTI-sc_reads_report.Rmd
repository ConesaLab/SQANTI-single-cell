...
render_ggplotly(gg_JCs_in_cell, target_width = 400, target_height = 600)
```
...
<div id="readspergene-dropdown" class="dropdown-toolbar">
  <div class="btn-group">
    <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
      <span class="label">
        `r if(mode == "reads") paste("% Annotated Genes by", entity_label, "Count Bins") else paste("%", entity_label_plural, "from Known/Novel Genes")`
      </span> <span class="caret"></span>
    </button>
    <ul class="dropdown-menu dropdown-menu-right" role="menu">
      `r if(mode == "isoforms") paste0('<li><a href="#" data-target="#readspergene-perc-reads-annot">% ', entity_label_plural, ' from Known/Novel Genes</a></li>')`
...
<div id="readspergene-perc-reads-annot" class="toggle-pane `r if(mode == 'reads') 'hidden-pane'`" style="display:`r if(mode == 'reads') 'none' else 'block'`">
...
<div id="readspergene-bins-all" class="toggle-pane `r if(mode == 'isoforms') 'hidden-pane'`" style="display:`r if(mode == 'isoforms') 'none' else 'block'`">
...
show_ujcs_gene <- (mode != "isoforms")
 ```
 
 ```{=html}
 <div id="ujcspergene-dropdown" class="dropdown-toolbar" style="`r if(!show_ujcs_gene) 'display:none;'`">
   <div class="btn-group">
     <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
       <span class="label">
...
## Coding Prediction {`r if(mode != "isoforms" || !exists("gg_coding_by_category")) "style='display:none;'"`}

```{r coding-check, include=FALSE}
show_coding <- (mode == "isoforms" && exists("gg_coding_by_category"))
```
...
if (exists("gg_coding_by_category")) {
  render_ggplotly(gg_coding_by_category)
}
```
...
if (exists("gg_noncoding_by_category")) {
  render_ggplotly(gg_noncoding_by_category)
}
```
...
if (knitr::is_html_output()) {
  ...
  render_ggplotly(gg_umap, target_width = 600, target_height = 500) %>%
